...
typedef struct bk_rlk_config_info {
    ...
    uint8_t format;             /**<  TX format */
    uint8_t band_width;         /**<  TX band width */
    uint8_t gi_type;            /**<  TX guard interval type */
    uint8_t ht_mcs;             /**<  TX HT MCS index */
 ...
 *
 * @param prim_chan   primary channel of Raw Link
 * @param second_chan secondary channel of Raw Link
 *
 * @return
 *    - BK_OK: succeed
 *    - others: other errors
 */
bk_err_t bk_rlk_set_channel(uint8_t prim_chan, uint8_t second_chan);
...
/**
 * @brief     Set BK Raw Link format
 *
 * @param     format  format
 *
 * @return
 *          - BK_OK : succeed
 *          - others : Set format error
 */
bk_err_t bk_rlk_set_format(uint8_t format);

/**
 * @brief     Set BK Raw Link band width
 *
 * @param     bw  band width
 *
 * @return
 *          - BK_OK : succeed
 *          - others : Set band width error
 */
bk_err_t bk_rlk_set_band_width(uint8_t bw);

/**
 * @brief     Set BK Raw Link GI type
 *
 * @param     gi_type  GI type
 *
 * @return
 *          - BK_OK : succeed
 *          - others : Set GI type error
 */
bk_err_t bk_rlk_set_gi_type(uint8_t gi_type);

/**
 * @brief     Set BK Raw Link HT MCS
 *
 * @param     mcs  HT MCS
 *
 * @return
 *          - BK_OK : succeed
 *          - others : Set HT MCS error
 */
bk_err_t bk_rlk_set_ht_mcs(uint8_t mcs);

/**
 * @brief     Set BK Raw Link ps interval
 *
 * @param     interval  ps interval
 *
 * @return
 *          - BK_OK : succeed
 *          - others : Set ps interval error
 */
bk_err_t bk_rlk_set_ps_interval(uint32_t ps_interval);

/**
 * @brief     Set BK Raw Link ps window
 *
 * @param     window  ps window
 *
 * @return
 *          - BK_OK : succeed
 *          - others : Set ps window error
 */
bk_err_t bk_rlk_set_ps_window(uint32_t ps_window);

/**
 * @brief     Get BK Raw Link ps interval
 *
 * @param     void
 *
 * @return
 *          - VALUE : ps interval
 */
uint32_t bk_rlk_get_ps_interval(void);

/**
 * @brief     Get BK Raw Link ps window
 *
 * @param     void
 *
 * @return
 *          - VALUE : ps window
 */
uint32_t bk_rlk_get_ps_window(void);
...
typedef void (*bk_rlk_sync_cb_t)();

bk_err_t bk_rlk_bypass_whitelist_check(bool enable);

bk_err_t bk_rlk_use_legacy_mac_header(bool enable);

bk_err_t bk_rlk_use_ext_feature(bool enable);
...
bk_err_t bk_rlk_sync(uint16_t interval, const void *data, size_t len, uint8_t mac_type, uint8_t *oui);

typedef void (*non_standard_slave_cb)(bool sosp);
bk_err_t bk_non_standard_master_init(uint8_t max_tx_num, uint8_t max_rx_num);
bk_err_t bk_non_standard_slave_init(uint8_t slave_mode, non_standard_slave_cb cb);
bk_err_t bk_non_standard_slave_connect();
bk_err_t bk_non_standard_slave_send(uint8_t *data, uint16_t len);
