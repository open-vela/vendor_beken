...
extern void *g_wifi_os_funcs;
extern void *g_wifi_os_variable;
/**
...
typedef enum {
	WIFI_BAND_POLICY_2G_ONLY = 0,
	WIFI_BAND_POLICY_5G_ONLY = 1,
	WIFI_BAND_POLICY_BI_BAND = 2,
} wifi_band_policy_t;
#endif
...
typedef enum {
	...
	EVENT_WIFI_START,              /**< WiFi start event */
	EVENT_WIFI_STOP,               /**< WiFi stop event */

	EVENT_WIFI_AP_START,            /**< WiFi AP start event */
	EVENT_WIFI_AP_STOP,             /**< WiFi AP stop event */

	EVENT_WIFI_READY,              /**< WiFi ready event */
	...
	EVENT_WIFI_AP_STAIPASSIGNED,   /**< The BK AP STA IP assigned */
	...
	EVENT_WIFI_DTIM_RECEIVED,      /**< The BK STA receives DTIM from AP */
	EVENT_WIFI_AWARE_WAKE_INTERVAL_START, /**< The BK Aware wake interval start */
...
typedef enum {
	...
	WIFI_REASON_SECURITY_LEVEL_NOT_MATCH = 261,      /**<The AP's security level is too low**/
...
typedef enum {
	...
	WIFI_CAPA_ID_TX_USE_MAX_BW, /**< Update Wi-Fi TX Use max bandwidth capability */
	WIFI_CAPA_ID_MAX,
}wifi_capability_t;
...
typedef struct {
	...
	uint8_t is_bridge;                 /**< is AP support bridge */
...
#define WIFI_AP_HW_MODE_B         (AP_HW_MODE_B)
#define WIFI_AP_HW_MODE_B_G       (AP_HW_MODE_B | AP_HW_MODE_G)
#define WIFI_AP_HW_MODE_B_G_N     (AP_HW_MODE_B | AP_HW_MODE_G | AP_HW_MODE_N)
#define WIFI_AP_HW_MODE_B_G_N_AX  (AP_HW_MODE_B | AP_HW_MODE_G | AP_HW_MODE_N | AP_HW_MODE_AX)
#define WIFI_AP_HW_MODE_A         (AP_HW_MODE_A)
#define WIFI_AP_HW_MODE_A_N       (AP_HW_MODE_A | AP_HW_MODE_N)
#define WIFI_AP_HW_MODE_A_N_AC    (AP_HW_MODE_A | AP_HW_MODE_N | AP_HW_MODE_AC)
#define WIFI_AP_HW_MODE_A_N_AC_AX (AP_HW_MODE_A | AP_HW_MODE_N | AP_HW_MODE_AC | AP_HW_MODE_AX)
#endif

#if CONFIG_WIFI_WIRELESS_MODE_SUPPORT
enum wifi_sta_hw_mode_t {
	STA_HW_MODE_B  = (1 << 0),
	STA_HW_MODE_G  = (1 << 1),
	STA_HW_MODE_N  = (1 << 2),
	STA_HW_MODE_A  = (1 << 3),
...
#define WIFI_WIRELESS_MODE_B      (STA_HW_MODE_B)
#define WIFI_WIRELESS_MODE_BG     (STA_HW_MODE_B | STA_HW_MODE_G)
#define WIFI_WIRELESS_MODE_BGN    (STA_HW_MODE_B | STA_HW_MODE_G | STA_HW_MODE_N)
#define WIFI_WIRELESS_MODE_BGNAX  (STA_HW_MODE_B | STA_HW_MODE_G | STA_HW_MODE_N | STA_HW_MODE_AX)
#define WIFI_WIRELESS_MODE_A      (STA_HW_MODE_A)
#define WIFI_WIRELESS_MODE_AN     (STA_HW_MODE_A | STA_HW_MODE_N)
#define WIFI_WIRELESS_MODE_ANAC   (STA_HW_MODE_A | STA_HW_MODE_N | STA_HW_MODE_AC)
#define WIFI_WIRELESS_MODE_ANACAX (STA_HW_MODE_A | STA_HW_MODE_N | STA_HW_MODE_AC | STA_HW_MODE_AX)
#endif
...
typedef struct {
...
#if CONFIG_WIFI_AP_HW_MODE
	uint8_t hw_mode;                   /**< Hardware mode of BK AP, default value TODO */
#endif
	...
	uint8_t bridge: 1;                 /**< BK AP support bridge */
...
typedef struct {
	...
	uint32_t last_scan_req;/**< last scan request */
...
typedef struct {
	...
	int freq;                         /**< frequency of connected AP*/
} wifi_event_sta_connected_t;

typedef struct {
	int disconnect_reason;                /**< Disconnect reason of BK STA */
	bool local_generated;                 /**< if disconnect is request by local */
	char    ssid[WIFI_SSID_STR_LEN];      /**< SSID of disconnected AP */
	uint8_t bssid[WIFI_BSSID_LEN];        /**< BSSID of disconnected AP*/
} wifi_event_sta_disconnected_t;

typedef struct {
	uint8_t mac[WIFI_MAC_LEN];            /**< MAC of the STA connected to the BK AP */
	uint32_t ipaddr;                      /**< IP address of the STA connected to the BK AP */
} wifi_event_ap_connected_t;

typedef struct {
	uint8_t mac[WIFI_MAC_LEN];            /**< MAC of the STA connected to the BK AP */
	uint32_t ipaddr;                      /**< IP address assigned to the STA (network order) */
} wifi_event_ap_staipassigned_t;
...
typedef struct {
	...
	uint8_t format;         /**< RAW Data TX format information */
	uint8_t band_width;     /**< RAW Data TX band width information */
	uint8_t gi_type;        /**< RAW Data TX guard interval type information */
	uint8_t ht_mcs;         /**< RAW Data TX HT MCS index information */
} raw_tx_cntrl_t;
...
typedef struct {
	uint32_t frequency; /**< frequency of the signal */
	int current_signal; /**< current signal */
	int avg_signal;     /**< average signal */
	int current_noise;  /**< current noise */
} wifi_signal_info_t;
...
/**
 * @brief wifi beacon miss cb interface
 */
