 ...
 *    - others: other errors.
 */
bk_err_t bk_gpio_wakeup_interrupt_clear(void);
#endif
...
/**
 * @brief     Map a device to a GPIO pin
 *
 * @param gpio_id GPIO pin to map the device to (must be < SOC_GPIO_NUM)
 * @param dev     Device type to map (must not be GPIO_DEV_INVALID)
 *
 * @return
 *    - BK_OK: succeed
 *    - others: failed
