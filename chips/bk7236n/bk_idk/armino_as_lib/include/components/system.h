...
typedef enum {
    ...
    RESET_SOURCE_ILLEGAL_PERSIST_DATA       = 0x21,
...
#define BK_ERR_INVALID_CPU_CLK    (BK_ERR_MAC_BASE - 4)
...
__attribute__((weak)) uint64_t bk_get_tick(void);
uint64_t bk_get_second(void);
uint32_t bk_get_ms_per_tick(void);
__attribute__((weak))  uint32_t bk_get_ticks_per_second(void);
int bk_get_tick_timer_id(void);
void bk_printf(const char *fmt, ...);
void bk_print_dump_hex(const uint8_t *ptr, size_t buflen);
void bk_print_dump_hex_16bit(const uint8_t *ptr, size_t buflen);
void bk_print_dump_hex_32bit(const uint8_t *ptr, size_t buflen);
...
void stop_cpu1_handle_notifications(void);
