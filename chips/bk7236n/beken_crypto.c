/****************************************************************************
 * vendor/beken/chip/bk7236n/beken_crypto.c
 *
...
static uint32_t g_beken_sesnum = 0;
...
static int beken_newsession(uint32_t *sid, struct cryptoini *cri)
{
...
static int beken_freesession(uint64_t tid)
{
  return 0;
}
...
static int beken_process(struct cryptop *crp)
{
      ...
      switch (crd->crd_alg)
        {
          ...
          case CRYPTO_AES_CTR:
            ...
            memcpy(iv, crd->crd_key + crd->crd_klen / 8 - AESCTR_NONCESIZE,
                   AESCTR_NONCESIZE);
            memcpy(iv + AESCTR_NONCESIZE, crp->crp_iv, AESCTR_IVSIZE + 4);
...
void hwcr_init(void)
{
  ...
  crypto_register(hwcr_id, algs, beken_newsession,
                  beken_freesession, beken_process);
}
